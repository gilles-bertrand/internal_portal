...
Pour les champs référentiels, le formulaire construit des options `{value: code, label}` depuis les enregistrements reçus en argument (`this.args.purposes`, `.legalBases`, `.dataCategories`) — pas de clé i18n `options.<champ>.<code>`, le libellé vient du backend, choisi entre `label` et `labelEn` selon la locale active (voir [[access-record-options#Libellés bilingues résolus côté frontend]]). Le `selectedItemComponent` (trigger affiché) résout le libellé du code stocké en le recherchant dans la liste d'options courante, via une fabrique `selectedOptionComponent(getOptions)` plutôt qu'une classe fixe par champ, pour rester générique aux trois champs.
...
`accessType` était seul en Approche A (valeur stockée = libellé affiché, `string[]` brut) — même pattern que dans `incident-form.gts` (`@libs/incident-registry-front/src/components/forms/incident-form.gts`, non lié en wiki-link : `.gts` non supporté par `lat check`). Il est passé en Approche B : la valeur persistée reste le code (`consultation`, `transmission`…, inchangé en base), mais l'affichage passe par une clé i18n, avec `@selectedItemComponent` comme les référentiels. Motif : rendus bruts, ces codes affichaient des mots français minuscules dans une interface anglaise. Le même traitement a depuis été appliqué aux quatre enums d'`incident-form.gts` — voir [[incident-registry-display#Options métier traduites, code persisté]].

## Libellés bilingues résolus côté frontend

Les référentiels exposent deux libellés (`label` français, `labelEn` anglais nullable) et le frontend choisit à l'affichage, jamais le backend à la sérialisation.

Le sélecteur de langue du dashboard change la locale **sans rechargement** : un libellé résolu côté serveur via `Accept-Language` resterait figé dans la langue du fetch. La résolution vit donc dans [[@libs/access-registry-front/src/utils/referential-label.ts#localizedLabel]], qui retombe sur le français quand `labelEn` est nul — cas des systèmes sources créés à la volée depuis le formulaire, où l'utilisateur ne saisit qu'un libellé. Contrat backend : [[backend/access-registry#Référentiels bilingues (label / labelEn)]].

Un access-record ne persiste que le **code** du référentiel. Toute vue qui l'affiche doit donc le retraduire : c'est le rôle de [[@libs/access-registry-front/src/utils/referential-label.ts#labelForCode]], qui rend le code tel quel si le référentiel a disparu — un référentiel supprimé ne doit pas effacer la donnée du registre. Avant cela, la vue détail et le tableau affichaient les codes bruts (« support », « art6.1b »).

Le cache des quatre référentiels vit dans le service `referentials` (`@libs/access-registry-front/src/services/referentials.ts`, non lié en wiki-link : résolution `.ts` ambiguë avec l'entité backend homonyme), chargé par les routes `index` et `show` plutôt que par chaque composant. Le formulaire l'invalide après création d'un système source, sinon le tableau continuerait d'afficher le nouveau système sous son code.

`accessType` n'a pas de référentiel : ses libellés sont des clés i18n `access-records.options.accessType.<code>`, dérivées par [[@libs/access-registry-front/src/utils/access-record-options.ts#accessTypeLabelKey]]. Les vues testent `intl.exists` avant traduction, pour qu'un code historique hors enum reste lisible au lieu d'afficher une clé manquante.
