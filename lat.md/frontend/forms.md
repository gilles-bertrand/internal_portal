...
`incident-form.gts` (wizard à 8 étapes) suit désormais la même convention : lien « retour au registre » avec `ArrowLeftIcon` à côté des boutons d'étape, et titre de page porté par les templates de route (`incidents.pages.create.title` / `incidents.pages.edit.title`, en `text-2xl` comme les autres) plutôt que par le composant — le wizard sert les deux modes. Il conserve en plus son `<h2>` par étape, qui titre le panneau courant et non la page.
