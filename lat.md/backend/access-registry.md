...
La liste ([[@libs/access-registry-backend/src/utils/pdf-table.ts#drawSummaryTable]]) utilise [[@libs/access-registry-backend/src/utils/pdf-summary.ts#buildSummaryCells]] (colonnes `SUMMARY_COLUMNS` : seq, date, personne, type, finalité, art. 9, système source, hash abrégé). Le détail ([[@libs/access-registry-backend/src/utils/pdf-detail-render.ts#drawRecordsDetail]]) utilise [[@libs/access-registry-backend/src/utils/pdf-detail.ts#buildRecordDetailRows]] — liste exhaustive label→valeur (séquence, dates d'accès/encodage, encodeur, accédant, personne concernée, catégories, art. 9, type, finalité, base légale, système source, destinataire, conservation, justification, `prevHash` et `hash` **complets**) rendue en blocs encadrés.

Densité : dans chaque fiche, les champs courts sont disposés sur **2 colonnes** ; la justification (texte libre) et les hash occupent la pleine largeur. Un champ libre coincé dans une demi-colonne se repliait sur 3-4 lignes et faisait exploser la fiche pour rien.
...
## Export PDF : mise en page mesurée

Aucune hauteur du PDF n'est écrite en dur : chaque cellule est **mesurée** avant d'être posée, parce que `lineBreak: false` ne protège pas du repli de texte dans pdfkit.

Le piège est la cause commune de tous les défauts d'affichage constatés à l'export. Dans pdfkit, `lineBreak: false` sert seulement à ne PAS déduire la largeur de la marge courante : dès qu'une `width` explicite est passée à `text()`, le line-wrapper s'active quand même et replie le texte. Les gabarits qui combinaient `width` + `lineBreak: false` en supposant « une ligne, quoi qu'il arrive » écrivaient donc plusieurs lignes dans un espace calculé pour une seule.

Conséquences observées, toutes corrigées : une justification longue s'écrivait par-dessus les lignes `HASH PRÉCÉDENT` / `HASH` de la fiche (les lignes de la grille étaient positionnées à `index * LINE_HEIGHT` fixe) ; une date `jj/mm/aaaa hh:mm` (58,4 pt en Helvetica 7,5) débordait de la colonne `Date` du récapitulatif (66 pt, padding déduit : 58 pt) et sa seconde ligne passait sous le filet de séparation ; le cadre de l'attestation, à hauteur fixe, ne suivait pas un motif de rupture verbeux.

### Troncature à la mesure

[[@libs/backend-shared/src/pdf/text.ts#fitText]] coupe une valeur à la largeur disponible (dichotomie sur `widthOfString`) et suffixe une ellipse ; [[@libs/backend-shared/src/pdf/text.ts#drawSingleLine]] la dessine **sans** `width`, seul moyen d'empêcher pdfkit de replier.

Ces helpers vivent dans `@libs/backend-shared` et sont partagés avec le registre des incidents ([[incident-registry#Export PDF : lignes de table mesurées]]). C'est ce qui remplace l'ancienne troncature au nombre de caractères de `buildSummaryCells` : une limite en caractères ignore la largeur des glyphes, donc tronquait à tort les valeurs étroites et laissait déborder les larges. `buildSummaryCells` renvoie désormais les valeurs complètes et la colonne décide. Corollaire à connaître : `drawSingleLine` n'avance pas `doc.y` (pdfkit décale `doc.x` quand `lineBreak: false` et qu'aucune `width` n'est donnée), donc [[@libs/access-registry-backend/src/utils/pdf-table.ts#drawSectionTitle]] pose son ordonnée suivante à la main — sans quoi le premier bloc de la section se dessine par-dessus le titre.

### Blocs mesurés et sécables

[[@libs/access-registry-backend/src/utils/pdf-detail-blocks.ts#buildBlocks]] transforme les champs d'un enregistrement en blocs `{ height, draw(y) }` dont la hauteur est connue **avant** tout tracé : une ligne de grille prend la hauteur de sa cellule la plus haute, une fiche prend la somme de ses blocs.

Un bloc de champ libre ([[@libs/access-registry-backend/src/utils/pdf-detail-blocks.ts#textBlock]]) est en plus **sécable** : son texte est pré-découpé en lignes par [[@libs/backend-shared/src/pdf/text.ts#wrapLines]] (pdfkit sait replier un texte mais ne dit pas où), ce qui permet à `split(maxHeight)` de rendre la moitié qui tient et de renvoyer le reste. [[@libs/access-registry-backend/src/utils/pdf-detail-render.ts#drawRecordsDetail]] enchaîne : une fiche qui tient sur une page n'est jamais coupée ; une fiche plus haute qu'une page se poursuit dans un cadre « (suite) » au lieu d'écrire sous la marge basse. La boucle de pagination force un bloc sur une page vierge, ce qui garantit sa convergence même sur un contenu aberrant. Régressions couvertes par [[@libs/backend-shared/tests/unit/pdf-text.test.ts]] et par les cas « justification longue » / « plus longue qu'une page » / « attestation compromise verbeuse » de [[@libs/access-registry-backend/tests/unit/export-pdf.test.ts]].

⚠️ Le registre des **incidents** ([[@libs/incident-registry-backend/src/utils/export-registry-pdf.ts]] et [[@libs/incident-registry-backend/src/utils/export-incident-pdf.ts]]) porte encore le même gabarit `width` + `lineBreak: false` à hauteurs fixes : il est exposé aux mêmes chevauchements et n'a pas été repris ici.
